 ...
 * RESTORATION GUIDE
 *
 * This component was removed in favor of the DataForSEO keywords
 * implementation.
 * To restore this component's functionality, the following changes
 * need to be made:
 *
 * 1. In greater-pitchedby.js:
 *    - Add back the cloud-platform scope to OAuth initialization:
 *      "https://www.googleapis.com/auth/cloud-platform"
 *    - This scope is required for the Google Cloud Natural Language API
 *
 * 2. In worker.js:
 *    - Restore the Language API endpoint:
 *      if (url.pathname === "/api/language/analyze" && request.method === "POST") {...}
 *    - This endpoint proxies requests to the Google Cloud Natural Language API
 *
 * 3. In Google Cloud Console:
 *    - Re-enable the Cloud Natural Language API
 *    - Ensure the API key has access to the Natural Language API
 *
 * 4. Sheet Structure:
 *    - The Keywords tab expects columns: "Keyword", "Type", "Relevance"
 *    - These columns are populated with entity analysis results
 *    - Ensure these columns exist when restoring functionality
 *
 * 5. Error Handling:
 *    - This component uses the greater-error event system
 *    - Verify the greater-errors component is present and functioning
 *
 * 6. Dependencies:
 *    - Requires Lit library: lit-core.min.js
 *    - Requires sheets-api-manager.js for Google Sheets operations
 */
...
export class GreaterNLPAnalyze extends LitElement {
  static properties = {
      ...
      try {
        ...
        if (entities.length > 0) {
          await this.updateValues(
            this.sheetId,
            "Lead Gen!A2:C" + (entities.length + 1),
            entities,
          );
        }
...
customElements.define("greater-nlp-analyze", GreaterNLPAnalyze);
